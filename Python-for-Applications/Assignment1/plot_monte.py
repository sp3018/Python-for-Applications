...
def monte_pi(darts):
    ...
    for i in range(darts):
        pointx = (random.randint(0,100)/100);
        pointy = (random.randint(0,100)/100);
        t.goto(pointx, pointy);
        if(insideCircle(pointx, pointy)):
        #color blue, point is inside the circle
            points+=1;
            t.color('blue');
            t.dot()
        else:
    ...
    # Apologies if this is not the formula that was in the book, I have yet to obtain it.
    pi=(points/darts)*4;
    print("Approximation of pi is %f" %(pi));
