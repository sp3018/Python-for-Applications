def filter_words_from_file(fn, rules=lambda word:True):
    ...
    :rtype: list
    """
    reader = open("wordsEn.txt", "r").read().splitlines();
    content = reader;
    filtered = [];
    filtered = [item for item in content if rules(item)];
    return filtered;
...
def load_password_file(pw_fn, delimiter=':'):
    ...
    :rtype: list of tuples
    """
    passwords = open(pw_fn, "r").read().splitlines();
    passwordTuples=tuple(passwords);
    ...
    * call generate_password_file with the rules function that you've 
    ...
    def rules(word):
        if(len(word) >= 14):
            if(word.count('s')>=2):
                if(word.count('o')>=1):
                    if(word.find('h')==0 or word.find('v')==0):
                        return True;

    generate_password_file("wordsEn.txt", "passwords.txt", rules);
